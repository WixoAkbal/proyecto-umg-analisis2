# Nombre: Luis Fernando Pérez Carrillo - 2294-09-2462 

## Rol: Devops, Desarrollador y QA

### Desarrollador en Java
### Desarrollador de micro servicios en Spring-Boot
### Diseñador de paginas web 
### Desarrollador de aplicaciones web modernas en Angular
### Administrador de base de datos Oracle/MariaDB/MySQL
### Administrador de servidores Linux
### Ingeniero de nube junior en AWS
### Desarrollador de reportes en Jaspersoft y Eclipse BIRT
